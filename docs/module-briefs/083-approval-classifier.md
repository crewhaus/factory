...
Workspace home: not created yet; likely packages/approval-classifier unless folded into a target bundle
Targets: CLI, CHN
Test layers: T1, T5, T8

## Purpose

ML auto-approve for safe ops (yolo classifier).
...
`claude-code/.../utils/permissions/yoloClassifier.ts`; `openclaw/.../acp/approval-classifier.ts`

Research focus: Calibration; abstain rate

## Validation Plan

Catalog tests: T1, T5, T8. Primary risks: security boundaries and policy bypasses; quality regressions that only show up through eval or trace grading.
