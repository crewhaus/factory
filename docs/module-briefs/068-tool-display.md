...
Workspace home: not created yet; likely packages/tool-display unless folded into a target bundle
Targets: CLI, CHN
Test layers: T1, T3

## Purpose

Render tool calls/results in TUI/UI (React/Ink).
...
`openclaw/.../agents/tool-display.ts`; `claude-code/Tool.ts` `renderToolUseMessage`

Research focus: Render perf; long-output truncation
