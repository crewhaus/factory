...
Workspace home: not created yet; likely packages/scheduler-cron unless folded into a target bundle
Targets: CHN, CLI, RES
Test layers: T1, T7

## Purpose

Cron service: at/every/cron schedules, persistence, run/wake/enqueue.
...
`openclaw/.../cron/service.ts`,`service/ops.ts`,`types.ts`

Research focus: Drift; missed-fire policy
