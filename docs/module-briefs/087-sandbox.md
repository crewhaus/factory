...
Status: planned
Dependency phase: 7 - Permission/policy
Catalog layer: R8 - Permission, Policy, Safety
Origin in ordering: inferred from catalog layer R8
Workspace home: not created yet; likely packages/sandbox unless folded into a target bundle
Targets: CLI, CHN, MGD, EVAL, BATCH
Test layers: T1, T3, T7, T8

## Purpose

Containerized exec environment (Docker/firecracker/wasm/chroot).
...
Inputs are tool intents, content, identities, policy rules, and approval decisions. Outputs are allow/deny/ask decisions, redacted content, and audit-ready reasons.

## Dependency Notes

Build this after the stable contracts from phases 1 through 6. Within phase 7, earlier numeric briefs are the preferred stabilization order, but independent modules can still be developed in parallel if their write sets do not overlap.
...
`openai-agents/.../sandbox/`; `openclaw/.../security/exec-policy.ts`; OpenAI Codex sandboxes

Research focus: VM startup latency; warm pool

## Validation Plan

Catalog tests: T1, T3, T7, T8. Primary risks: security boundaries and policy bypasses; load, long-run behavior, and storage pressure.
