...
Status: planned
Dependency phase: 15 - Channels
Catalog layer: R13 - Channels & Messaging
Origin in ordering: named in Part G
Workspace home: not created yet; likely packages/channel-features unless folded into a target bundle
Targets: CHN
Test layers: T1, T3

## Purpose

Cross-channel features (typing, edit-in-place, mention-gate, allowlist, model-override, debounce, threads).
...
Inputs are channel events, user identities, media payloads, and outbound messages. Outputs are normalized messages and delivery state.

## Dependency Notes

Build this after the stable contracts from phases 1 through 14. Within phase 15, earlier numeric briefs are the preferred stabilization order, but independent modules can still be developed in parallel if their write sets do not overlap.
...
`openclaw/.../channels/plugins/` features

Research focus: Capability-based feature gating
