...
Workspace home: not created yet; likely packages/permission-engine unless folded into a target bundle
Targets: CLI, CHN, CRW, MGD, BATCH
Test layers: T1, T3, T8

## Purpose

Modes (`default`/`plan`/`auto`/`bypass`); rule sources (command/settings/claudemd/hook/policy); rule types (alwaysAllow/alwaysDeny/alwaysAsk).
