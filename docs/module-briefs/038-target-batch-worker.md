...
Workspace home: not created yet; likely packages/target-batch-worker unless folded into a target bundle
Targets: **BATCH**
Test layers: T1, T3, T7

## Purpose

Codegen for queue/batch worker: queue consumer, dedup, retry, rate limit, observability.
...
None mainstream - design from primitives. Loose ref: `dramatiq/`/`celery/`/`temporal` patterns; `claude-code/.../tools/MonitorTool` (background loop pattern)

Research focus: Queue protocol abstraction; idempotency keys
