...
Workspace home: not created yet; likely packages/batch-runner unless folded into a target bundle
Targets: BATCH
Test layers: T1, T3, T7

## Purpose

Drive worker loop: pull -> process -> ack -> repeat.
...
`temporal`/`celery`/`dramatiq` patterns

Research focus: Concurrency; back-off
