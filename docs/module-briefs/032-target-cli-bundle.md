...
Codegen for CLI coding agent: entry, REPL, tools, hooks, settings.json, MCP config. Emits grouped tool imports + `defaultCatalog.register()` calls + `tools: defaultCatalog.list()` for any spec with non-empty `tools`; unknown names fail at compile time via `TargetEmitError`. Hooks/settings/MCP still pending.
...
`claude-code/main.tsx`, `claude-code/cli/`, `claude-code/entrypoints/`

Research focus: Bundling (Bun/Node); side-effect imports

## Validation Plan

Catalog tests: T1, T3, T7. Primary risks: load, long-run behavior, and storage pressure.
