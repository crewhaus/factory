...
Workspace home: not created yet; likely packages/bootstrap-files unless folded into a target bundle
Targets: CLI, CHN, RES
Test layers: T1, T3, T9

## Purpose

Workspace-file injection (CLAUDE.md/AGENTS.md/TOOLS.md/USER.md/SOUL.md/HEARTBEAT.md), budget, caching.
...
`openclaw/.../agents/bootstrap-files.ts`,`bootstrap-cache.ts`,`bootstrap-budget.ts`; `claude-code/context.ts`

Research focus: Walk semantics; budget allocation

## Validation Plan

Catalog tests: T1, T3, T9. Primary risks: edge cases best caught by fuzz or property checks.
