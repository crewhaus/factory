...
Workspace home: not created yet; likely packages/vector-store unless folded into a target bundle
Targets: RAG, CHN, CRW, RES
Test layers: T1, T2, T7

## Purpose

Vector index over memory/docs (FAISS/qdrant/pinecone/weaviate/local).
...
`haystack/.../document_stores/`; `llama_index/.../vector_stores/`

Research focus: Backend abstraction; metadata filtering
