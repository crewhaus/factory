...
Catalog layer: R20 - Batch-Worker Specific
Origin in ordering: expanded from batch stack
Workspace home: not created yet; likely packages/fan-out-fan-in unless folded into a target bundle
Targets: BATCH, CRW
Test layers: T1, T3, T7

## Purpose

Split a large job into many tasks; aggregate results.
...
`agent-framework/.../orchestrations/`; `temporal` workflows

Research focus: Aggregation guarantees
