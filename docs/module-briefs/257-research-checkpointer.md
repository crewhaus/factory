...
Workspace home: not created yet; likely packages/research-checkpointer unless folded into a target bundle
Targets: RES
Test layers: T1, T4, T7

## Purpose

Stage-level checkpoints for hours-long runs (resume after crash/preempt).
...
`agent-framework/.../_checkpoint.py` (ref); `langgraph` durability

Research focus: Stage granularity
