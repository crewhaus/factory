...
Workspace home: not created yet; likely packages/global-state-singleton unless folded into a target bundle
Targets: CLI, CHN
Test layers: T1, T3

## Purpose

Process-global registry (`Symbol.for(...)`).
...
`openclaw/global-state.ts`; `claude-code/.../bootstrap/state.ts`

Research focus: Test isolation
