...
Workspace home: packages/tool-fs
Targets: CLI, CHN, CRW, RES, BATCH
Test layers: T1, T3, T8

## Purpose

`read`/`write`/`edit`/`apply_patch`/glob/grep over workspace; FS policy enforcement.
...
`claude-code/.../tools/FileReadTool` etc.; `openclaw/.../agents/bash-tools.*`; `openclaw/.../agents/tool-fs-policy.ts`; `openai-agents/.../sandbox/files.py`

Research focus: Atomic edit; path traversal defense
