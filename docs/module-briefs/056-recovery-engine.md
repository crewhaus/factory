...
Workspace home: not created yet; likely packages/recovery-engine unless folded into a target bundle
Targets: CLI, CHN, CRW, GRPH, MGD, RES
Test layers: T1, T4

## Purpose

`max_output_tokens` retries, `prompt_too_long` collapse, media-error strip, model-fallback tombstoning.
...
`claude-code/query.ts` recovery branches; `agent-framework/.../_runner.py` retry

Research focus: Recovery taxonomy; idempotence under retry
