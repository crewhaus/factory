...
Origin in ordering: inferred from catalog layer R1
Workspace home: not created yet; likely packages/durability-mode unless folded into a target bundle
Targets: GRPH, MGD, CHN, RES
Test layers: T1, T4, T7

## Purpose

Configurable durability: `exit` / `async` / `sync` checkpoint write.
...
`langgraph/.../checkpoint` durability modes

Research focus: Async write semantics; loss-window analysis
