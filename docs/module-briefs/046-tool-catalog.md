...
Workspace home: packages/tool-catalog
Targets: CLI, CHN, CRW, MGD, GRPH, RES, BROW, BATCH
Test layers: T1

## Purpose

Registry of tools w/ metadata (concurrency-safe, read-only, destructive, defer, profile).
...
`claude-code/Tool.ts` + `tools.ts`; `openclaw/.../agents/tool-catalog.ts`; `openai-agents/.../tool.py`

Research focus: Metadata schema; profile membership
