...
Workspace home: not created yet; likely packages/subagent-runtime unless folded into a target bundle
Targets: CLI, CRW, MGD, RES
Test layers: T1, T3, T6

## Purpose

Spawn subagents w/ filtered tool sets and forked context.
...
`claude-code/.../tools/AgentTool/`; `claude-code/.../utils/agentContext.ts`; `openclaw/.../agents/pi-embedded-runner/`; `crewAI/.../agent`; `openai-agents/.../_agent.py`; `adk-python/.../agents/`

Research focus: Context filter rules; isolation
