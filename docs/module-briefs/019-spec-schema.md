...
Workspace home: packages/spec
Targets: All
Test layers: T1, T9

## Purpose

Define user-facing harness spec DSL (YAML/TS): agents, tools, channels, workflow, eval, deploy. Versioned, JSON Schema-backed. Currently `name` + `target: cli` + `agent` + optional `tools: string[]`.
...
`agent-framework/python/.../declarative/`, `adk-python/cli/`, `openclaw.yaml` schema, `crewAI/.../agent` configs, `claude-code/.../settings.ts`

Research focus: DSL design tradeoffs (YAML vs TS); how MAF declarative format vs ADK YAML differ; LangGraph imperative vs declarative split
