...
Workspace home: not created yet; likely packages/feature-flags unless folded into a target bundle
Targets: All
Test layers: T1

## Purpose

Build-time + runtime flags (dead-code elimination).
...
`claude-code/main.tsx` `bun:bundle feature()` pattern

Research focus: Build-time elim
