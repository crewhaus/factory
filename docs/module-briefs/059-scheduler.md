...
Workspace home: not created yet; likely packages/scheduler unless folded into a target bundle
Targets: All
Test layers: T1, T7

## Purpose

Concurrency primitives: bounded parallelism, queue, priority lanes (UI vs background vs cron).
...
`openclaw/.../cron/isolated-agent` lane mgmt

Research focus: Lane isolation; starvation prevention
