...
Workspace home: packages/compiler
Targets: All
Test layers: T1, T3

## Purpose

Orchestrate frontend (parse -> validate -> IR) and dispatch to target backends; emit deployment bundle. `lower()` threads `spec.tools ?? []` into `ir.tools`.
...
LangChain LCEL -> LangGraph compile path; MAF `_workflow_builder.py`; ADK declarative-config compilation

Research focus: Pipeline composition; error reporting; partial compilation
