...
| Module | Responsibility | Refs | Targets | Research focus | Tests |
|---|---|---|---|---|---|
| **`mcp-host`** | MCP client manager: stdio/SSE/websocket transports, reconnect, OAuth, server registry. | `claude-code/.../services/mcp/`; `openclaw/mcp/`; `agent-framework/.../_mcp.py`; `openai-agents/.../mcp/`; `adk-python/.../tools/` MCP support | All except pure RAG/EVAL | Transport handshake; reconnection backoff | T1, T2, T7, T8 |
...
| Module | Responsibility | Refs | Targets | Research focus | Tests |
|---|---|---|---|---|---|
| **`session-store`** | Short-term session state, identity, lifecycle. | `openclaw/sessions/`; `claude-code/.../utils/sessionStorage.ts`; `agent-framework/.../_sessions.py`; `adk-python/.../sessions/`; `openai-agents/.../memory/session.py` | All | TTL; eviction; identity model | T1, T3, T7 |
| **`event-log`** | Append-only event/transcript history (JSONL). | `claude-code/.../utils/sessionStorage.ts`; `AI-Harness-Systems.md` §append-only event history; OpenClaw transcripts | All | Schema versioning; GC | T1, T4, T7 |
...
| **`state-store`** | In-process state container (zustand-style); referential equality, listeners. | `claude-code/.../state/store.ts` (40 lines) | All | Observers; immutability | T1, T9 |
...
| Module | Responsibility | Refs | Targets | Research focus | Tests |
|---|---|---|---|---|---|
| **`permission-engine`** | Modes (`default`/`plan`/`auto`/`bypass`); rule sources (command/settings/claudemd/hook/policy); rule types (alwaysAllow/alwaysDeny/alwaysAsk). | `claude-code/.../utils/permissions/` (14 files); `AI-Harness-Systems.md` §Policy engine | CLI, CHN, CRW, MGD, BATCH | Layered evaluation; precedence | T1, T3, T8 |
