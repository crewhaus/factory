...
- **Model routing / per-model settings / hybrid setups?** → `packages/model-service/` (the `wireModels` composition root every routed emitter and the `crewhaus run` interpreter call — the `modelFallbacks` / `circuitBreaker` / `modelTiers` / `modelPool` fragment is rendered and wired in exactly one place), `packages/model-plan/` (the pure per-candidate primitives), `packages/model-router/`, `packages/routing-store/`. The 0.6.0 plan's rule: every new routing feature lands in `model-service`, never in an emitter's codegen.
