...
3. **Destructive or visible-side-effect tools should set `requireJustification: true`** — `SendMessage`, `EvmSendTransaction`, `HttpRequest`, and federation outbound tools all do. The builtin table's `justify` column mirrors the flag, and `apps/cli/src/tool-registry.test.ts` holds the two together, so a bundle README reports the gate the tool really has.
...
- **New tool?** → `packages/tool-builder/`, `packages/tool-catalog/` — remember to set `scope: "external"` if the tool crosses a network/process boundary, `requireJustification: true` if it has destructive or visible side effects. [module-briefs/047-tool-builder.md](https://github.com/crewhaus/docs/blob/main/module-briefs/047-tool-builder.md).
  A new **built-in** tool is one row in the builtin table, `BUILTIN_TOOLS` in [packages/tool-categories/src/builtins.ts](packages/tool-categories/src/builtins.ts) — package, export, registered name, the io/sandbox/justify facts, and the boot registrar that receives its `tool_config` (`initSymbol`) or its chain blocks (`chainSymbol`) — plus its category and its suggest keywords. A package that reads configuration exports ONE registrar, listed in `TOOL_BOOT_REGISTRARS` with the `tool_config` key its README documents; `config.ts` beside it is the one rule every emitter, `crewhaus run`, `crewhaus eval` and a model pool candidate use to pick the block, and `apps/cli/src/boot-seams.test.ts` fails for any boot seam in a tool package that nothing delivers. Every shape's emitter, `crewhaus run`, `crewhaus eval`, `lint` and the `tools` commands read that table (through `resolveBuiltinTools` and the per-shape profiles in `shapes.ts`); no emitter keeps a map of its own, and `apps/cli/src/shape-tools.test.ts` fails if one reappears. Do not edit the table by hand: write a manifest and run `bun run scripts/wire-tool-package.ts <manifest.json>`, which writes the row (reading the name and facts off the tool), the category, the keywords, the CLI's package loader and the build references from one declaration, and is idempotent. `apps/cli/src/tool-registry.test.ts` then holds both directions — every categorized key is a real builtin, every builtin is categorized, and every row's facts match the tool — so a tool added without wiring fails a test rather than going quietly missing. A tool only some shapes can carry gets `shapes: [...]` on its row instead of a private map.
