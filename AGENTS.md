...
**Intent gate** — orthogonal third layer. Tools that set `requireJustification: true` (for example `SendMessage`, `EvmSendTransaction`, `HttpRequest`, `EmailSend`) demand the model supply a `justification` string with each call. The contract is advertised, not just enforced (#386): at request-build time `runChatLoop` augments the gated tool's model-facing input schema with a required `justification` string property (unless the tool's own schema already declares one) and strips a runtime-injected field from the input again before it reaches the tool's validator or a remote MCP server — see `withJustificationField`/`stripJustificationField` in [packages/tool-catalog](packages/tool-catalog). `permission-engine`'s `evaluateJustification` checks it against the spec's `instructions` (the goal an attacker cannot influence) via a configurable judge — rule-based by default (`ruleBasedJustificationJudge`), model-backed in production (`@crewhaus/justification-judge-claude`, selected via `runChatLoop({ justificationJudge })`, the spec's `security.justification.judge`, or `--justification-judge`). The gate publishes a `permission_decision` trace event (carrying first-class `judgeModel` + `justificationConfidence`) for live observability AND — when a durable sink is wired via `runChatLoop({ justificationAuditSink })` — appends a hash-chained `permission_justification_evaluated` record (verbatim `{ toolName, justification, verdict, reason, judgeModel, confidence? }`) to `@crewhaus/audit-log`. The CLI `run` path opens that sink at `.crewhaus/audit` by default (opt out with `--no-justification-audit`). SACR's three-layer model (deterministic governance → behavioral analysis → non-deterministic governance) lands its third layer here; recipe [demos/walkthroughs/53-justification-gates.md](https://github.com/crewhaus/demos/blob/main/walkthroughs/53-justification-gates.md) walks through it.

**Contributor rules:**

1. **Any new module that ingests external content registers a `TrustOrigin`** in [packages/boundary-classifier/src/index.ts](packages/boundary-classifier/src/index.ts) (the union AND its `ORIGIN_DEFAULT_POLICY` row; the mirrored union in `packages/run-context` and the egress severity matrix are compile-checked against it, and the test arrays derive from `TRUST_ORIGINS` — the six-site checklist is in the union's docblock), calls `classifyBoundary` before the content reaches a model call or a tool result, AND calls `tagContent(ctx, content, origin)` after a non-blocked verdict so the sink-side classifier sees it.
...
3. **A destructive tool that goes to a place the model chose sets `requireJustification: true`.** "A place the model chose" is `scope: "external"` plus a `url` or `recipient` operative argument (`hasModelChosenDestination` in [packages/tool-catalog](packages/tool-catalog)): the tool sends, posts or writes where the model pointed it — `HttpRequest`, `EmailSend`, `ChatPost`, the issue and pull-request writers, `DownloadFile`, `SendMessage`, `EvmSendTransaction`. Other tools may opt in when their effect deserves an intent check (fleet mutations, secret rotation). A destructive tool that acts only inside the workspace (`Write`, `RemovePath`, `Bash`) is not required to: the permission gate already asks, and without an LLM judge a justification fails closed in production. Two companion rules for the other flags: a tool that spawns a program the workspace supplies (a project's linter, a binary in `node_modules/.bin`) is never `readOnly`, because plan mode runs read-only tools without asking; and a tool that is neither read-only nor destructive runs in auto mode without asking, so it must be one you would let run unasked. [apps/cli/src/flag-rules.test.ts](apps/cli/src/flag-rules.test.ts) holds all three over every builtin and lists the tools each one lets through, so a new tool on the permissive side is a written-down decision.
...
- **No new package without a module brief** in [module-briefs](https://github.com/crewhaus/docs/tree/main/module-briefs). Briefs document responsibilities, depended-on / unblocks, and the catalog layer. The 0.6.0 model-plan train's packages — `packages/model-plan` (brief 307), `packages/model-service` (308) and `packages/tool-consult` (309) — are documented by briefs landing in crewhaus/docs as the release's companions (plan §12). `packages/qr-code` (brief 313), the dependency-free QR encoder behind `crewhaus hangar --lan`, follows the same rule.
...
- **New tool?** → `packages/tool-builder/`, `packages/tool-catalog/` — remember to set `scope: "external"` if the tool crosses a network/process boundary, `operativeArgs` for the argument(s) that decide where it acts, and `requireJustification: true` if it is destructive and goes to a place the model chose (contributor rule 3 above). [module-briefs/047-tool-builder.md](https://github.com/crewhaus/docs/blob/main/module-briefs/047-tool-builder.md).
  A new **built-in** tool also has to be registered in five places that must agree — the category registry, `target-cli`'s `BUILTIN_TOOL_MAP`, the CLI's `loadToolMap`, `CLI_RUNTIME_TOOL_KEYS`, and the suggest keyword table. Do not edit them by hand: write a manifest and run `bun run scripts/wire-tool-package.ts <manifest.json>`, which makes all five edits from one declaration and is idempotent. `apps/cli/src/tool-registry.test.ts` then holds both directions — every categorized key is a real builtin, and every builtin is categorized — so a tool added without wiring fails a test rather than going quietly missing.
  `packages/tool-text` is the reference for a **deterministic** tool package: pure functions in `src/lib/` where the behaviour is tested, thin tool wrappers in `src/index.ts`, and a package-wide test asserting the safety flags, so a future tool that reaches outside has to change that assertion deliberately.
...
- **New external sink / egress check?** → `packages/egress-classifier/`, mark the tool `scope: "external"`, [recipes/55-egress-fabric.md](https://github.com/crewhaus/demos/blob/main/walkthroughs/55-egress-fabric.md).
...
- **Model routing / per-model settings / hybrid setups?** → `packages/model-service/` (the `wireModels` composition root the `crewhaus run` / `serve` interpreter calls, and whose codegen twin `renderModelWiringFields` every routed emitter renders — the `modelFallbacks` / `circuitBreaker` / `modelTiers` / `modelPool` fragment is rendered and wired in exactly one place; under `strategy.model_directed` `wireModels` also constructs the `Consult` / `Escalate` tools, and a compiled bundle registers them through `wireHybrid` on every pool-bearing shape. The one standing exception is a `pipeline` pool's `model_directed`, which plan §11.3 marks `—`: the compiler reports it as `model-plan-ignored-on-shape`, and since `crewhaus run` refuses a pipeline spec outright the key is inert everywhere, not just in the bundle), `packages/tool-consult/` (the two model-directed tools: a nested single-turn side call on an allowlisted roster sibling, classified at TrustOrigin `"consult"`, and the self-escalation receipt + latch), `packages/model-plan/` (the pure per-candidate primitives), `packages/model-router/`, `packages/routing-store/`. The 0.6.0 plan's rule: every new routing feature lands in `model-service`, never in an emitter's codegen.
- **Memory / continuity fabric?** → `packages/memory-service/` (the `wireMemory` composition root every emitter calls), `packages/continuity-store/`, `packages/wiki-store/`, `packages/dream-engine/`; the `continuity:`/`memory:`/`thredz:`/`learning:` spec blocks lower to `IrContinuity`/`IrMemory`/`IrThredz`/`IrLearning` and collapse to one `wireMemory` call. See COMPILER-ARCHITECTURE.md "Memory & continuity lowering".
