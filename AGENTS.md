...
- **No new package without a module brief** in [module-briefs](https://github.com/crewhaus/docs/tree/main/module-briefs). Briefs document responsibilities, depended-on / unblocks, and the catalog layer. The 0.6.0 model-plan train's packages — `packages/model-plan` (brief 307), `packages/model-service` (308) and `packages/tool-consult` (309) — are documented by briefs landing in crewhaus/docs as the release's companions (plan §12).
