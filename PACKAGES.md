...
- **Deterministic tools** — tools that reach no model and, in the `compute` group, no I/O at all: `@crewhaus/tool-text` (regex, diff, templating, classification), `@crewhaus/tool-data` (JSON/YAML/TOML/CSV/XML), `@crewhaus/tool-encode` (hashing, HMAC, URLs, identifiers), `@crewhaus/tool-datetime` (dates, durations, cron), `@crewhaus/tool-schema` (validation and assertions), `@crewhaus/tool-flow` (branching, decision tables, error classification, deadlines, consensus, stall detection), `@crewhaus/tool-pkg` (semver resolution, lockfile diffs, license rollups, tarball inspection, publish preflight), `@crewhaus/tool-money` (payment identifier checksums, tax, refund allocation, three-way match, cost basis, spend limits, statement parsing), `@crewhaus/tool-onchain` (ABI encoding and decoding, EIP-55 addresses, EIP-712 digests, token units, fixed-point DeFi maths — offline, and it never signs), `@crewhaus/tool-html` (CSS selection, tables, links, forms, structured data and readable text, over markup rather than a live browser), `@crewhaus/tool-verify` (golden comparison with visible normalization, checksum manifests, acceptance checks, Markdown link checking, citation linting), `@crewhaus/tool-table` (profiling, keyed reconciliation, record linkage, contact normalization, reshaping, fixed-width parsing). They exist so a harness spends a model call on judgement rather than on mechanism.
- **Tool categories** — `@crewhaus/tool-categories` carries the category registry behind the `tools: [all-<category>, -<tool>]` selector grammar. Data only, by design: the compiler expands the grammar at lower() time, so it must not pull a tool implementation into the compile path.
- **Builtin tool manifest** — `@crewhaus/tool-registry-manifest` carries every builtin tool's name, description, flags, categories and keywords as generated data, so a compiled bundle can describe a tool it was not given. Dependency-free for the same reason tool-categories is. Regenerate with `bun run scripts/gen-tool-registry.ts`; `apps/cli/src/tool-registry.test.ts` fails when it has gone stale.
- **Tool capability** — `@crewhaus/tool-capability` exports the `ToolRegistry` tool, which reads that manifest against the live catalog so a harness can see both the tools it has and the ones it does not. An ergonomics feature, not a control: it reports, and enables nothing.
...
- **Model routing & hybrid setups (v0.6.0)** — `@crewhaus/model-plan` (the pure plan primitives), `@crewhaus/model-service` (the composition root every emitter and both interpreters call), `@crewhaus/tool-consult` (the `Consult` / `Escalate` pair), `@crewhaus/model-router`, `@crewhaus/routing-store`
- **Observation & learning** — `@crewhaus/watchme-store` (the durable observation/judgment store and cross-harness registry behind `crewhaus watchme`)
...
- `@crewhaus/studio-server`, `@crewhaus/studio-ui`, `@crewhaus/studio-plugin-sdk`, `@crewhaus/scaffold-templates`, `@crewhaus/wizard`, `@crewhaus/trace-viewer`, `@crewhaus/graph-visualizer`, `@crewhaus/dataset-builder`, `@crewhaus/grader-builder`, `@crewhaus/spec-forms`
- `@crewhaus/vscode-extension`, `@crewhaus/jetbrains-plugin`
- `@crewhaus/crewhaus-playground` (browser REPL)

All thirteen publish in their own lockstep (currently 0.1.5), independent of the factory cut.
...
2. **Auth.** In CI there is nothing to export — the Release workflow's `npm` job runs with `permissions: id-token: write`, and npm exchanges that OIDC token for a short-lived, package-scoped publish credential. **The workflow's filename is load-bearing:** every package's trusted publisher is bound to `(crewhaus/factory, release.yml)`, npm does not validate that binding when it is saved, and a mismatch surfaces only as a publish-time 404/ENEEDAUTH. Renaming `release.yml` invalidates every one of those configs at once.

   For a local publish, or for a package whose trusted publisher is not configured yet, fall back to a **granular** access token (Read and write) with **bypass two-factor authentication**, scoped to the whole **`@crewhaus` scope** *plus* the unscoped **`crewhaus`** package — a token scoped to a list of existing names cannot create a package that does not exist yet. Write tokens cap at 90 days. Classic *Automation* tokens are not an option: npm permanently revoked every one of them on 2025-12-09, and they cannot be recreated or recovered — that revocation, not an expiry, is why v0.4.1 and v0.4.2 failed to publish.
3. **Canary.** `bun scripts/publish-workspace.ts --filter @crewhaus/errors` — one leaf package proves auth before the full fan-out.
4. **Publish.** `bun scripts/publish-workspace.ts` — topological order, skips versions already on the registry. Publishes with `npm publish` (adding `--provenance` when OIDC is available). The old "must stay on `bun publish`" rule is retired: it existed only because bun resolved `workspace:*` at pack time and npm shipped the literal range, and step 1's `--for-publish` now does that resolution from the stamped version. The script refuses to publish any manifest still carrying a `workspace:` range, or whose internal deps disagree with the version being cut.
