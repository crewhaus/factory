...
2. **Auth.** In CI there is nothing to export — the Release workflow's `npm` job runs with `permissions: id-token: write`, and npm exchanges that OIDC token for a short-lived, package-scoped publish credential. **The workflow's filename is load-bearing:** every package's trusted publisher is bound to `(crewhaus/factory, release.yml)`, npm does not validate that binding when it is saved, and a mismatch surfaces only as a publish-time 404/ENEEDAUTH. Renaming `release.yml` invalidates all 214 configs at once.
