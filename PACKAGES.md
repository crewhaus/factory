...
- **Deterministic tools** — tools that reach no model and, in the `compute` group, no I/O at all: `@crewhaus/tool-text` (regex, diff, templating, classification), `@crewhaus/tool-data` (JSON/YAML/TOML/CSV/XML), `@crewhaus/tool-encode` (hashing, HMAC, URLs, identifiers), `@crewhaus/tool-datetime` (dates, durations, cron), `@crewhaus/tool-schema` (validation and assertions). They exist so a harness spends a model call on judgement rather than on mechanism.
