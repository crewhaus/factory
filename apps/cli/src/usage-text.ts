...
export function usageText(): string {
  return [
    ...
    "       [--model-plan]                  arrive as PRs for human review — never auto-merged).",
    "                                       --suite appends a nightly `eval suite --tier nightly",
    "                                       --gate` step to the same cron (NEW-HUNT-8);",
    "                                       --model-plan also scaffolds crewhaus-model-plan.yml,",
    "                                       the nightly `models audit --propose` / `route propose`",
    "                                       job that opens a PR and never merges (0.6.0 §9.1)",
    "  init [name]                          scaffold a new crewhaus.yaml (--hybrid writes the",
    "       [--hybrid]                       models: registry + two-arm pool + cascade instead)",
    ...
    "  models list | explain [<spec>]       the resolved models: registry; every slot's profile,",
    "                                       the hybrid strategy in one sentence, and this shape's",
    "                                       carry/emit/ignore verdict (0.6.0 §8.2)",
    "  models audit [<spec>]                walk every model slot: pricing, capabilities vs",
    "       [--fail-on none|pricing|sunset] `requires`, per-(provider,knob) parameter acceptance.",
    "       [--today YYYY-MM-DD]            EXITS 1 on a model already past its retirement date;",
    "       [--propose] [-o <dir>]          --propose writes the replacement patch per slot",
    "  models propose --source sunset       the replacement bundle, or (--source audition) a roster",
    "       | audition [--min-n N]          PR bundle for a shadow arm past the n>=30 power floor.",
    "                                       Never applied — the roster is human-owned (§9.3)",
    "  route status [--dir <root>]          show the adaptive model_pool reward scoreboard",
    "       [--by profile|scope] [--shadow] regroup by arm/scope; --shadow reveals the observe-only",
    "       [--json]                        q:/shadow: lanes (hidden by default: they steer nothing)",
    "  route explain <session> [--dir <r>]  replay a run's routing timeline — the model_route",
    "       [--json]                        decisions with the /model directives and hybrid stage",
    "                                       lines interleaved in turn order",
    "  route propose [-o <dir>] [--json]    mine the scoreboard into WHITELISTED pool patches, as the",
    "                                       suggestions.json `optimize --from-advice` eval-gates",
    ...
    "  services setup <spec.yaml>           provision the external services the spec asks for:",
    "       [--services slack,cloudflare,thredz]   a Slack app (manifest + install), a Cloudflare",
    "       [--zone <domain>] [--hostname <fqdn>]  named tunnel + public hostname + DNS, and a",
    "       [--tunnel <name>] [--port <n>]         Thredz wiki space. Credentials land in the",
    "       [--space-type shared|individual]       harness .env; the wiki slug lands in the spec.",
    "       [--app-id <id>] [--manual-install]     Provisioning tokens are read once, never stored.",
    "       [--env-file <path>] [--dry-run] [-y]",
    ...
    "       [--group <name>]                --group keeps only harnesses in a registry group",
    "                                       FILESYSTEM-centric (walks --root); `harness` is the",
    "                                       registry-centric twin — both are supported",
    "  harness <verb> ...                   the machine-wide harness registry + manager (Hangar):",
    "       list [--group G] [--json]        every registered harness, joined with inventory",
    "       show <dir|id> [--json]           one harness: registry entry + inventory + health",
    "       add <dir> | remove <dir|id>      register manually / drop the registry row only",
    "       relocate <id> <newDir>           point an entry at a moved directory (same id)",
    "       group <name> [--add|--remove <dir|id>] [--color C] [--order N]   manage groups",
    "       tag <dir|id> --add|--remove <t> | pin <dir|id> [--off]           tags + pinning",
    "       scan [--root <dir>]              discover harnesses under scan roots + upsert",
    "       preflight <dir|id> [--json]      will-it-boot checks (exit 1 on blocking items)",
    "                                       REGISTRY-centric (machine-wide, any location); `fleet`",
    "                                       above is the filesystem-walk twin",
    "  hangar [serve]                       boot the Hangar manager console (local web UI over the",
    "       [--port N] [--host H]           registry; single-instance hangar.lock; the auth token",
    "       [--no-auth] [--no-open]         reaches the browser as a URL #fragment, never a query",
    "       [--read-only]                   string; --host requires auth AND opting in with",
    "       [--read-only-locked] [--smoke]  CREWHAUS_HANGAR_ALLOW_REMOTE=1; --read-only boots with",
    "                                       every mutating route refused (--read-only-locked also",
    "                                       refuses the un-toggle, so only a restart lifts it);",
    "                                       --smoke = ephemeral boot + self-check, the release",
    "                                       workflow's smoke entry)",
    "  hangar status [--json] | open        is the console running? / print + reopen its URL",
    "  daemon <verb> [<dir|hrn_id>] ...     supervise a harness process from the terminal (M2);",
    "       start [--force] [--ack <ids>]    preflight gates every start — missing channel secrets",
    "       [--no-preflight]                 can NEVER be forced (the daemon exits 2 on them)",
    "       stop | restart | status [--json] runfile/liveness/control port + recent runs",
    "       logs [--tail N] [--follow]       the SCRUBBED captured output of a run",
    "       [--run <run_id>]",
    "       wake --lane heartbeat|schedule   one synthetic tick down the daemon's OWN timer path,",
    "       [--reason R] | drain             via crewhaus.control.v1; drain = stop intake, exit 0",
    "                                       No <dir|hrn_id> means the current directory. These",
    "                                       verbs drive the supervisor directly, so they work",
    "                                       with no Hangar console running (and either head",
    "                                       adopts what the other started).",
