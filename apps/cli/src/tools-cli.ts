...
export const TOOL_KEYWORDS: Readonly<Record<string, ReadonlyArray<string>>> = Object.freeze({
  ...
  leadAssign: ["assign", "round robin", "territory", "owner", "routing"],
  sequenceRun: ["sequence", "next step", "dependencies", "plan", "prerequisites"],
  localTime: ["local time", "wall clock", "timezone now", "business hours", "what time is it"],
  emitTraceEvent: ["emit event", "trace", "telemetry", "instrument", "custom event"],
  emailSendPreflight: ["preflight", "before sending", "recipients", "dry run email", "envelope"],
  deliverabilityCheck: ["deliverability", "SPF", "DKIM", "DMARC", "mail dns"],
  vectorDelete: ["delete vectors", "remove embeddings", "vector store", "purge namespace"],
  factCrossCheck: [
    "cross-check",
    "fact check",
    "claim",
  ...
  marketplaceSearch: ["template search", "marketplace", "find a starter", "template registry"],
  federationDiscover: ["federation", "peers", "discover agents", "remote harnesses"],
  routeControl: ["routing", "promote lane", "model routing", "route policy"],
  experimentLedger: ["experiment", "assignment", "variant", "a/b ledger"],
  flywheelStatus: ["flywheel", "improvement loop", "status"],
  watchmeReport: ["watchme", "observational learning", "co-learning", "recall"],
  specPin: ["pin a spec", "deploy version", "environment pin", "tenant overlay"],
  deployRollback: ["rollback", "revert deployment", "previous version"],
  deployInspect: ["what is deployed", "deployment status", "pinned where", "environments"],
  harnessRegister: ["register harness", "harness registry", "relocate harness", "harness groups"],
  harnessJobStatus: ["job status", "harness jobs", "queue status", "job ledger"],
  compileBundle: ["compile", "recompile bundle", "bundle staleness", "rebuild harness"],
  cliVersionPin: ["cli version", "pinned version", "fleet versions", "chvm"],
  hooksManage: ["manager hooks", "postCompile", "preSpawn", "hook command"],
  harnessRetire: ["retire a harness", "decommission", "archive harness"],
  storeMigrate: ["migrate store", "store version", "move harness data"],
  retentionEnforce: ["retention", "delete old data", "age out", "data retention policy"],
  knowledgeSync: [
    "knowledge sync",
    "share memories",
    "pull shared prompts",
  ...
  approvalStatus: ["approval status", "what is parked", "waiting for approval"],
  approvalsInbox: ["approvals inbox", "fleet approvals", "pending across harnesses"],
  permissionsSuggest: ["suggest permissions", "permission rules", "stop the prompts", "allow rule"],
  datasetPut: ["version a dataset", "dataset put", "register dataset", "dataset split"],
  datasetInspect: ["inspect dataset", "dataset stats", "what is in this dataset"],
  datasetLint: ["dataset lint", "duplicate samples", "split leakage", "dataset validation"],
  datasetMine: ["mine sessions", "grow a dataset", "harvest cases", "dataset from production"],
  evalHistory: ["eval history", "trend", "runs over time", "score history"],
  evalAggregate: ["aggregate results", "pass rate", "recompute eval", "slice results"],
  evalBaselinePin: ["pin a baseline", "eval baseline", "regression gate"],
  evalCoverage: ["eval coverage", "dataset coverage", "untested cases"],
  graderMetaTest: ["grader sanity", "meta test", "does the grader work", "grader calibration"],
  specPatchApply: ["patch a spec", "edit spec", "apply spec change", "optimizable path"],
  specUpgrade: ["upgrade notes", "spec migration", "post-release changes", "version upgrade"],
  specAdvise: ["spec advice", "harness findings", "what to change", "advisory rules"],
  doctorFix: ["doctor fix", "mechanical repair", "scaffold spec", "fix env stubs"],
  ...
  trashPath: ["trash", "recoverable delete", "move to trash", "recycle bin", "undoable delete"],
  osIndexSearch: [
    "spotlight",
    "mdfind",
    "locate",
  ...
  envFileUpsert: ["env file", ".env", "set env var", "update dotenv", "env upsert"],
  secretRotate: [
    "rotate a secret",
    "credential rotation",
    "replace a key",
  ...
  systemInfo: ["system info", "os version", "cpu", "memory", "disk", "uptime", "machine facts"],
  networkInfo: ["network interfaces", "ip address", "mac address", "mtu", "link status"],
  portInspect: ["listening ports", "open ports", "what is on port", "socket owner", "lsof"],
  objectPresign: [
    "presigned url",
    "S3",
    "SigV4",
  ...
  vatIdValidate: ["VAT", "VIES", "tax id", "EU VAT number", "vat validation"],
  ...
  sanctionsScreen: ["sanctions", "OFAC", "screening", "watchlist", "denied party", "name match"],
  eInvoiceBuild: ["e-invoice", "UBL", "factur-x", "EN 16931", "XRechnung", "invoice xml"],
  eInvoiceParse: [
    "read e-invoice",
    "parse UBL",
    "parse CII",
  ...
  paymentFileBuild: ["NACHA", "ACH", "SEPA", "pain.001", "payment batch", "payment file"],
  ledgerPost: [
    "double entry",
  ...
  ledgerQuery: ["trial balance", "account balance", "aging", "ledger query", "hash chain verify"],
  ledgerReconcile: ["reconcile", "bank statement", "match transactions", "unmatched", "near miss"],
  invoiceRender: [
    "invoice",
    "render invoice",
    "invoice number",
  ...
  priceQuote: ["price", "quote", "exchange rate", "asset price"],
  oraclePriceRead: ["chainlink", "pyth", "oracle", "price feed", "staleness", "heartbeat"],
  defiPositionRead: ["defi position", "aave", "lending", "health factor", "collateral"],
  portfolioValuation: ["portfolio", "valuation", "holdings", "total value", "unpriced"],
  tokenResolve: [
    "resolve token",
    "token symbol",
    "token address",
  ...
  erc20Balance: ["erc20 balance", "token balance", "decimals", "wallet balance"],
  erc721TokenInfo: ["nft", "erc721", "token uri", "nft owner", "token metadata"],
  evmMulticall: ["multicall", "batch calls", "aggregate3", "batched read"],
  contractInspect: [
    "contract inspect",
    "proxy detection",
    "implementation slot",
  ...
  gasMarketRead: ["gas price", "base fee", "priority fee", "eip-1559", "fee history"],
  evmGetBlock: ["evm block", "get block", "block by number", "chain block"],
  evmBlockAtTimestamp: ["block at time", "timestamp to block", "historical block", "block search"],
  evmRpcHealth: ["rpc health", "chain id", "archive node", "endpoint check", "node status"],
  evmNonceStatus: ["nonce", "transaction count", "pending nonce", "account nonce"],
  evmWaitForReceipt: ["wait receipt", "transaction mined", "confirmation", "tx status", "reverted"],
  ...
  evmEventScan: ["event logs", "eth_getLogs", "log scan", "contract events", "topic filter"],
  dataDriftCheck: [
    "data drift",
    "distribution shift",
    "PSI",
  ...
  containerImageTags: ["list image tags", "registry tags", "container tags", "tag listing"],
  dependencyAudit: [
    "vulnerability scan",
    "OSV",
    "CVE",
  ...
  registrySearch: ["search packages", "find a library", "registry search", "package discovery"],
  registryOutdated: [
  ...
  bundleSizeCheck: ["bundle size", "size budget", "size regression", "gzip size", "asset weight"],
  benchmarkCompare: [
    "benchmark",
  ...
  diffParse: ["parse diff", "unified diff", "patch hunks", "diff line numbers", "patch parse"],
