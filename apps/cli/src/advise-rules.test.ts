...
describe("ruleCompactionThrash", () => {
  ...
  it("fires at the edge (3 compactions in ONE session) with a curate patch", () => {
  ...
  it("falls back to tuning advice when curate is already enabled", () => {
    ...
    expect(suggestion?.kind === "advice" && suggestion.text).toContain("dedupeThreshold");
  });
});
