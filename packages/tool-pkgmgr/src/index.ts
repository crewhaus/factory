...
export const packageInstall: RegisteredTool = buildTool({
  ...
  execute: async (input, context) => {
    ...
    return json({
      ...
      ...(result.code !== 0 ? { reason: firstLine(result.stderr) || firstLine(result.stdout) } : {}),
      plan,
      before: factsJson(facts),
...
export const PKGMGR_TOOLS: readonly RegisteredTool[] = Object.freeze([packageQuery, packageInstall]);
