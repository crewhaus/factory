...
export function newContext(timeoutMs = DEFAULT_COMMAND_TIMEOUT_MS, signal?: AbortSignal): RunContext {
...
async function queryApt(
  ...
  if (!includeAvailable) {
    unknowns.add("available.version", "apt-cache policy", "the caller asked for installed state only");
  } else {
    ...
    if (policy.missing || policy.timedOut || truncated(policy)) {
      ...
      if (parsed === undefined) {
        ...
        knownToManager = true;
        available =
          parsed.candidate === null ? null : { version: parsed.candidate, source: "apt" };
