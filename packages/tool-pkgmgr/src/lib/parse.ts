...
export function parseDnfTransaction(stdout: string): DnfTransactionRow[] {
  ...
  let inSection = false;
  for (const line of lines(stdout)) {
    if (/^(Installing|Upgrading|Downgrading|Reinstalling)(\s+(weak\s+)?dependencies)?:\s*$/.test(line)) {
      inSection = true;
      continue;
    }
    if (/^\S/.test(line)) {
...
export function pacmanSaysNeedsRoot(text: string): boolean {
  return /you cannot perform this operation unless you are root|insufficient permission/i.test(text);
}
