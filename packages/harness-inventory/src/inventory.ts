 ...
 * inventory/health rollup. A team runs many harnesses (the Slack bot, the CLI
 * concierge, the nightly optimizer) each in its own directory per the
 * standalone-harness convention; this package discovers them under a root and
 * rolls their state up into one inventory / health table, and plans/executes a
 * bulk subcommand across the filtered set.
 *
 * Lifted verbatim from the CLI's `fleet` module (Item 58) so every surface
 * that needs the cross-harness view — `crewhaus fleet`, the harness manager,
 * remote consoles — consumes ONE implementation. Side-effect-free on import
