 ...
 *     samples against a non-chat-capable shape fail loudly at load.
 * Absent (plain `target: eval` specs), the emission is unchanged.
 */
export type EmitEvalOptions = EmitReadmeOptions & { readonly bridge?: EvalBridge };
  ...
  const bridgeImports =
    bridge === undefined
      ...
      : bridge.entryImport !== undefined
        ...
        : `import { guardHistorySamples } from "@crewhaus/target-eval-bundle";\n`;
  const bridgeConsts =
    bridge === undefined
      ? ""
      : `const BRIDGE = { sourceTarget: ${escapeJsonString(bridge.sourceTarget)}, kind: ${escapeJsonString(bridge.kind)}, chatCapable: ${bridge.chatCapable} } as const;\n${
          bridge.entryImport !== undefined
...
// Cluster S (D36 + NEW-shape-1) — the eval bridge's runtime helpers (imported
// by generated bridged bundles) and the eval-entry re-emission dispatch the
// CLI's `compile --with-eval-harness` path calls.
