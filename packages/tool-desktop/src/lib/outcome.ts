...
export type Unavailable = {
   ...
   * What was absent.
   *
   * `"unknown"` is a member in its own right and is NOT a spare: a probe that
   * timed out did not establish that a session, a program or a grant was
   * missing, and naming one of those would be the same class of false claim
   * rule 6 is about, one level down in the result. The reason string says
   * what could not be established.
   */
  readonly missing: "platform" | "program" | "session" | "permission" | "unknown";
  readonly reason: string;
...
export function unsupportedPlatform(platform: string, what: string): Unavailable {
  return unavailable("platform", `this package has no ${what} backend for platform "${platform}"`);
}
