...
/**
 * The Windows disambiguating probe, and the reason it exists.
 *
 * `Get-Clipboard -Raw` returns `$null` for an image or a file-drop exactly as
 * it does for an empty clipboard, so Windows has the SAME ambiguity macOS has
 * and an earlier draft of this file only handled the macOS half. This names
 * the non-text flavours without returning their content, mirroring
 * `clipboard info`.
 *
 * DOCUMENTED, NOT RECORDED: `-Format Image|FileDropList|Audio` is a
 * PowerShell 5.1 parameter set, but nobody here has a Windows box to capture
 * it on. It is written to fail safe — any error or unexpected output leaves
 * `classifyClipboardInfo` reading empty stdout, which is the answer the
 * primary read already gave.
 */
export const WINDOWS_CLIPBOARD_FORMATS = registerPowerShellScript({
  name: "clipboard-formats",
  script:
    "$f = @(); foreach ($k in 'Image','FileDropList','Audio') { try { if (Get-Clipboard -Format $k) { $f += $k } } catch {} }; [Console]::Out.Write($f -join ',')",
  reads: [],
});
...
export type ClipboardPlan =
  | {
       ...
       * clipboard from one holding a flavour the reader cannot return.
       * macOS and Windows both need it; the Linux backends report the
       * difference in their own stderr, so neither carries one.
       */
...
export function planClipboardRead(
  ...
  switch (platform) {
    ...
    case "win32": {
      const script = format === "html" ? WINDOWS_CLIPBOARD_READ_HTML : WINDOWS_CLIPBOARD_READ_TEXT;
      const built = powershellArgv(script, {});
      return {
        ok: true,
        backend: "powershell Get-Clipboard",
        request: { argv: built.argv },
        disambiguate: { argv: powershellArgv(WINDOWS_CLIPBOARD_FORMATS, {}).argv },
      };
    }
    default:
...
export function classifyClipboardRead(result: {
  ...
  for (const pattern of NO_SESSION) {
    if (pattern.test(result.stderr)) {
      return unavailable(
        "session",
        `the clipboard backend could not reach a display: ${result.stderr.trim()}`,
      );
    }
  }
