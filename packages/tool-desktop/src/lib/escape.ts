 ...
 *        on macOS 26.6.2 (Darwin 25.6.0) while writing this:
 *
 *          $ osascript -e 'on run argv' \
 *              -e 'return (item 1 of argv) & "|" & (item 2 of argv)' \
 *              -e 'end run' -- '-e' 'say "hi"; display dialog "x"'
 *          -e|say "hi"; display dialog "x"
 *
 *        Both hostile values came back as DATA. Nothing was spoken and no
 *        dialog appeared. Without the `--`, the same run answers
 *        "osascript: option requires an argument -- e", which is why the `--`
...
/**
 * Every registered PowerShell script's SOURCE.
 *
 * Exported for one assertion: `every powershell argv is a registered script
 * with nothing appended` in index.test.ts checks that the last element of
 * every recorded `powershell.exe` argv is one of these, and that there is
 * nothing after it. That narrow, checkable guarantee is what lets `run.ts`
 * carve PowerShell out of its blanket shell refusal.
 */
export function registeredPowerShellSources(): ReadonlySet<string> {
  return new Set([...POWERSHELL_SCRIPTS].map((s) => s.script));
}
 ...
 *
 * -25211 and -1728 are both reported by System Events for an un-granted
 * Accessibility request, which is why this is a set and not an equality
 * check. -1743 is the Automation (Apple-events) denial, a DIFFERENT grant in
 * a DIFFERENT pane of System Settings, so its message names the right one.
 */
...
export function osascriptPermissionReason(stderr: string): string | undefined {
  ...
  if (/not authori[sz]ed to send apple events/i.test(message)) {
 ...
 * by accident — the env keys supplied must match the script's declared
 * `reads` EXACTLY, so neither an unread extra nor a missing one gets through.
 */
...
/*
 * The three classes of character an XML 1.0 document cannot carry.
 *
 * WRITTEN AS `\u` ESCAPES, NEVER `\x` AND NEVER THE RAW BYTE. House rule 13,
 * and the earlier draft of this file broke it: it contained the literal bytes
 * 0x00, 0x08, 0x0B, 0x0C, 0x0E, 0x1F and 0x7F inside the character class, so
 * `file(1)` reported this source as "data" and `grep` refused to search it.
 * biome folds `\x` escapes into raw bytes on format, and bun 1.3.11 (the CI
 * runtime) then rejects the class as "range out of order" while the 1.3.14
 * here accepts it — a package that compiles on the author's machine and not
 * on the box that ships it.
 *
 * Kept as three separate one-line constants so the `biome-ignore` can sit
 * DIRECTLY above the only line that needs it. A suppression with anything in
 * between is reported as unused AND leaves the rule firing.
 */

// biome-ignore lint/suspicious/noControlCharactersInRegex: finding control characters is the entire job here.
const XML_ILLEGAL_C0 = /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g;
/** An unpaired surrogate is not a character; `LoadXml` throws on one. */
const XML_LONE_SURROGATE =
  /[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/g;
/** The two permanent non-characters at the end of the BMP. */
const XML_NONCHARACTER = /[￾￿]/g;

/**
 * Strip what an XML 1.0 document cannot represent.
 *
 * A C0 control such as U+0001 has no escape in XML 1.0 — the numeric
 * reference for it is itself illegal — so `LoadXml` throws on the document
 * rather than showing a broken toast. Dropping it is the only option that
 ...
 * legal and are kept.
 *
 * U+007F (DEL) is in fact LEGAL in XML 1.0 — it is above U+0020 — and is
 * dropped here anyway, because it is a discouraged character that no
 * notification is improved by and its removal is reported like any other.
 * Saying so is the point: an escaper whose comment overstates what it is
 * doing is one somebody later "simplifies" on the strength of the comment.
 */
export function stripXmlIllegalChars(value: string): { text: string; removed: number } {
  const cleaned = value
    .replace(XML_ILLEGAL_C0, "")
    .replace(XML_LONE_SURROGATE, "")
    .replace(XML_NONCHARACTER, "");
 ...
 *
 * `--` solves this wherever the program supports it. Verified on macOS
 * 26.6.2: `open -- -a` answers "The file /private/tmp/-a does not exist",
 * while `open -a` answers "open: option requires an argument -- a". Where a
 * program documents no `--` — CUPS `lp` and `lpstat` do not — the answer is
 * not to escape but to REFUSE, and to say so. This repo has already shipped
 * one argument injection of exactly this shape (`gitBranchCreate({name:"-D"})`
 * ran `git branch -D victim`).
 */
