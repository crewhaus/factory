...
export function planNotify(
  ...
  switch (platform) {
    ...
    case "linux": {
      ...
      if (env.DISPLAY !== undefined) displayEnv["DISPLAY"] = env.DISPLAY;
      if (env.WAYLAND_DISPLAY !== undefined) displayEnv["WAYLAND_DISPLAY"] = env.WAYLAND_DISPLAY;
      if (env.XDG_RUNTIME_DIR !== undefined) {
        displayEnv["XDG_RUNTIME_DIR"] = env.XDG_RUNTIME_DIR;
      }
      ...
      const argv = [
        ...
        `--expire-time=${Math.trunc(input.expireMs)}`,
        "--",
        input.title,
