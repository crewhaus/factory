 ...
 * dangling link is still a door. `open(…, "w")` through `evil -> /etc/x`
 * CREATES `/etc/x`. Probing with `lstat` keeps that name in the part of the
 ...
 * ancestor that EXISTS AS A NAME is resolved and the missing tail
 * re-appended. "Exists as a name" rather than "exists": a symlink pointing
 * at a file that is not there yet is still followed by `open(…, "w")`, so it
 * is resolved here too rather than treated as a plain missing leaf. Any
