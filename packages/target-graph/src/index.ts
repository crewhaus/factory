...
function renderNodeBody(
  ...
  // 0.6.0 §7.7 — the node's model-routing quartet (`modelFallbacks` /
  // `circuitBreaker` / `modelTiers` / `modelPool`) is rendered onto the
  // node's `runChatLoop` call below by `@crewhaus/model-service`'s
  // `renderModelWiringFields`, the one renderer every emitter shares (graph
  // nodes carried NO routing before 0.6.0). Empty when the node declares
  // none, so pre-0.6.0 bundles stay byte-identical.
  ...
  return `
    async (ctx, prev) => {${evalSeamBlock}${hitlPreBlock}${nudgeBlock}${forceBlock}
      ...
      const __reply = await runChatLoop({
        ...
        seedMessages: __seed,
        singleTurn: true,${renderNodeLoopFields(node)}${renderModelWiringFields(scopedModelWiringFragment(node, node.name), "        ")}${renderHybridWiringFields(scopedModelWiringFragment(node, node.name), "        ", node.name)}${forceField}${toolsField}${graphLevelFields}
...
function renderAgent(ir: IrGraphV0, evalEntry = false): string {
  ...
  // 0.6.0 PR 9d/9e — the composition root, imported only when a node's pool
  ...
  const hybridImport = Object.values(ir.nodes).some((n) => poolNeedsHybridWiring(n.modelPool))
    ? `\n${HYBRID_WIRING_IMPORT}`
    : "";
...
${errorsImport}${judgeImport}${hybridImport}
