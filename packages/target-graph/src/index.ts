...
function renderAgent(ir: IrGraphV0, evalEntry = false): string {
  ...
  // 0.6.0 PR 9d — the side-call composition root, imported only when a node's
  // pool declares a guide / shadow / committee (pre-9d bundles byte-identical).
  const sideCallImport = Object.values(ir.nodes).some((n) => hasSideCallStrategy(n.modelPool))
...
${errorsImport}${judgeImport}${sideCallImport}
