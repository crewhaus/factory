...
function renderAgent(ir: IrPipelineV0, evalEntry = false): string {
  ...
  const hybridFields = (indent: string): string =>
    renderHybridWiringFields(ir.agent, indent, ir.name, HYBRID_FAMILIES_BY_SHAPE.pipeline);
  ...
  instructions: ${escapeJsonString(ir.agent.instructions)},${renderModelWiringFields(ir.agent, "  ")}${hybridFields("  ")}${taxonomyField(ir, "  ")}
...
${hybridImport}${approvalImport}import { defaultCatalog } from "@crewhaus/tool-catalog";
${permImport}
