 ...
 * dangling link is still a door. A containment walk that probes with
 * `existsSync` strolls straight past `evil.txt -> /etc/x`, treats it as a
 * plain missing leaf, and re-appends the name to the realpath'd parent, so
 * the check passes on a path that actually points outside the workspace.
 * Probing with `lstat` — a dangling link is a name that exists — keeps that
 * name in the part of the path that gets RESOLVED rather than in the "does
 * not exist yet" tail that is appended to the root verbatim.
 */
...
function resolveLocation(target: string, depth = 0): string {
  ...
  try {
    ...
    probeReal = resolveLocation(resolve(base, link), depth + 1);
  }
  return tail.length > 0 ? join(probeReal, ...tail) : probeReal;
}
...
function resolveSafe(toolName: string, rel: string, root: string = process.cwd()): string {
  ...
  //    `resolveLocation` resolves the deepest ancestor that EXISTS AS A NAME
  //    and re-appends the missing tail. "Exists as a name" rather than
  //    "exists": a symlink whose target is not there yet is still followed by
  //    anything that opens the path, so it is resolved here too rather than
  //    treated as a plain missing leaf.
