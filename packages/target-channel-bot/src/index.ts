...
`;
  ...
  const approvalsResumeMethod = approvalsOn
    ...
    async resumeApproval(approval: PendingApproval, adapter: ChannelAdapter): Promise<void> {
      ...
      if (approval.status !== "grant") return;
      // A grant with no captured event — resolved by \`crewhaus approvals
      // grant\` from a terminal, or arriving after a restart cleared this
      // in-memory map — cannot re-drive the turn here, because the inbound
      // message that started it was never persisted. SAY SO. Returning
      // silently is why an operator who granted from the CLI saw nothing
      // happen at all and had no way to tell a working grant from a broken
      // one; the parked call is released and the NEXT inbound message
      // re-drives it pre-approved.
      if (event === undefined) {
        process.stdout.write(
          "[session-router] approval " + approval.id + " granted, but this process has no captured" +
            " inbound event for it (granted out of band, or the daemon restarted) — the grant is" +
            " live and the next message in that conversation runs pre-approved\\n",
        );
        return;
      }
      try {
