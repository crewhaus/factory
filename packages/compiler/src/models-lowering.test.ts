...
describe("model_pool candidates carry the merged profile (key order is the byte contract)", () => {
  ...
  test("the hybrid siblings lower (camelCase, role slots → tag or arm id, model slots resolved) and ride the blob", () => {
    ...
    const pendingPaths = warnings
      ...
      .map((w) => w.path);
    // PR 10 owns the reward store; everything else on a cli pool is wired.
    expect(pendingPaths).toContain("agent.model_pool.reward");
    // PR 9b consumes `directives` and `rules` straight off the blob on BOTH
    // paths; PR 9c consumes `strategy.cascade` + `max_escalations`; PR 9e
