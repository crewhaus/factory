...
export const seoLint: RegisteredTool = buildTool({
  name: "SeoLint",
  description:
    "Gate a page on the on-page SEO and readability checks that have a right answer offline: title and description lengths, heading hierarchy, image alt and anchor-text coverage, keyword placement and density, JSON-LD shape and the fields a rich result needs, canonical and OpenGraph consistency, slug shape, and near-duplicate text against a corpus. Use it before publishing. Three of these cannot be answered exactly, and each says so rather than pretending: readability is reported as a BAND and never as a number, because the syllable count behind Flesch-Kincaid is a heuristic and a decimal invites tuning prose against its bugs; keyword density DISABLES ITSELF for Chinese, Japanese, Thai and the other languages written without spaces between words, where a whitespace tokenizer is meaningless; and the near-duplicate check reports NOT CHECKED when no corpus is configured, rather than passing. JSON-LD is checked against rules bundled offline for the common types; any other @type is named as NOT CHECKED, never called valid. Everything a run could not look at is listed with a reason, and ok is false whenever anything went unchecked — a gate that passes on checks it did not make is worse than no gate.",
  inputSchema: z
    .object({
