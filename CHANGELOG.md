...
- **§56 Track E** — New `packages/meta-harness-optimizer`. **BREAKING (opt-in).** Coding-agent proposer with filesystem-backed full history. Default mutator stays `rule`; this is invoked via `crewhaus optimize --mutator meta-harness`. Bundle output diverges from spec; `formatBreakingChangeHeader()` prepends a warning. Source: Meta-Harness (arxiv 2603.28052).
...
- Full unit suite: 5896 pass / 0 fail / 2 skip across 430 files.
